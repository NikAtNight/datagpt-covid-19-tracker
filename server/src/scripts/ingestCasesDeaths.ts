...
async function ingestCasesDeathsData() {
	try {
		// Create table if not exists
		const schemaSQL = fs.readFileSync(path.join(process.cwd(), "src/db/schemas/casesDeaths.sql"), "utf-8");

		await pool.query(schemaSQL);

		// Clear existing data
		await pool.query("TRUNCATE TABLE covid_cases_deaths");

		// Read and parse CSV
		const parser = fs.createReadStream("../covid_data/cases_deaths/full_data.csv").pipe(
			parse({
				columns: true,
		...
		for await (const record of parser) {
			const query = `
				INSERT INTO covid_cases_deaths (
					date,
					location,
					new_cases,
					new_deaths,
		...
		console.log("Cases and deaths data ingestion completed successfully");
	} catch (error) {
		console.error("Error ingesting cases and deaths data:", error);
	} finally {
		await pool.end();
	}
}

ingestCasesDeathsData();
